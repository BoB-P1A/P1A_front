...
export default function SecurityReport() {
  ...
  const handleDownload = async () => {
    ...
    Object.keys(actionsByTarget).forEach(targetName => {
      sections.push(new Paragraph({ spacing: { before: 200, after: 100 }, children: [new TextRun({ text: `[${targetName}]`, bold: true })] }));
      const actionRows = [
        new TableRow({ children: [new TableCell({ children: [new Paragraph('코드')] }), new TableCell({ children: [new Paragraph('질의문')] }), new TableCell({ children: [new Paragraph('취약점')] }), new TableCell({ children: [new Paragraph('개선 가이드')] }), new TableCell({ children: [new Paragraph('조치 방안')] }), new TableCell({ children: [new Paragraph('조치 기간')] }), new TableCell({ children: [new Paragraph('부서')] }), new TableCell({ children: [new Paragraph('담당자')] }), new TableCell({ children: [new Paragraph('조치 일시')] })] }),
        ...actionsByTarget[targetName].map((act: any) => new TableRow({ children: [new TableCell({ children: [new Paragraph(act.code)] }), new TableCell({ children: [new Paragraph(act.question || '')] }), new TableCell({ children: [new Paragraph(act.evidence || '')] }), new TableCell({ children: [new Paragraph(act.guide || '')] }), new TableCell({ children: [new Paragraph(act.actionPlan || '')] }), new TableCell({ children: [new Paragraph(act.actionPeriod || '')] }), new TableCell({ children: [new Paragraph(act.department || '')] }), new TableCell({ children: [new Paragraph(act.manager || '')] }), new TableCell({ children: [new Paragraph(act.actionDate || '')] })] }))
      ];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>1. 영향평가 기준</CardTitle></CardHeader><CardContent className="space-y-2">{Object.keys(criteriaByTarget).map((target) => (<div key={target}><p className="font-semibold">[{target}]</p><ul className="list-disc pl-6">{Object.keys(criteriaByTarget[target]).map((sub) => (<li key={sub}>{sub} ({criteriaByTarget[target][sub].join(', ')})</li>))}</ul></div>))}</CardContent></Card>

      <Card><CardHeader><CardTitle>2. 평가기준에 따른 개인정보 침해요인 분석·평가</CardTitle></CardHeader><CardContent><Table><TableHeader><UITableRow><TableHead>검토대상명</TableHead><TableHead>질의문 코드</TableHead><TableHead>취약점</TableHead><TableHead>침해요인</TableHead></UITableRow></TableHeader><TableBody>{riskItems.map((r: any, i: number) => (<UITableRow key={i}><UITableCell>{r.targetName}</UITableCell><UITableCell>{r.code}</UITableCell><UITableCell>{r.evidence}</UITableCell><UITableCell>{r.riskFactor}</UITableCell></UITableRow>))}</TableBody></Table></CardContent></Card>

      <Card><CardHeader><CardTitle>3. 주요 위험요소에 따른 개선 조치 계획</CardTitle></CardHeader><CardContent className="space-y-4">{Object.keys(actionsByTarget).map((target) => (<div key={target}><p className="font-semibold mb-2">[{target}]</p><Table><TableHeader><UITableRow><TableHead>코드</TableHead><TableHead>질의문</TableHead><TableHead>취약점</TableHead><TableHead>개선 가이드</TableHead><TableHead>조치 방안</TableHead><TableHead>조치 기간</TableHead><TableHead>부서</TableHead><TableHead>담당자</TableHead><TableHead>조치 일시</TableHead></UITableRow></TableHeader><TableBody>{actionsByTarget[target].map((act: any, i: number) => (<UITableRow key={i}><UITableCell>{act.code}</UITableCell><UITableCell>{act.question}</UITableCell><UITableCell>{act.evidence}</UITableCell><UITableCell>{act.guide}</UITableCell><UITableCell>{act.actionPlan}</UITableCell><UITableCell>{act.actionPeriod}</UITableCell><UITableCell>{act.department}</UITableCell><UITableCell>{act.manager}</UITableCell><UITableCell>{act.actionDate}</UITableCell></UITableRow>))}</TableBody></Table></div>))}</CardContent></Card>
    </div>
  );
}
