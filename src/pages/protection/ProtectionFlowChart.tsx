...
export default function ProtectionFlowChart() {
  ...
  const handleSave = async () => {
    try {
      ...
      // 파일명 생성 (URL-safe)
      const companyId = sanitizeForStorage(user?.company || 'unknown');
      ...
      const { error: dbError } = await (supabase as any)
        .from('flow_charts')
        .upsert({
          company_id: user?.company || 'unknown',
