...
export default function ProtectionFlowTable() {
  ...
  const [flowDataByTask, setFlowDataByTask] = useState<Record<string, TaskFlowData>>(() => {
    ...
    Object.keys(savedData).forEach((taskName) => {
      migratedData[taskName] = {
        ...
        usage: savedData[taskName]?.usage || [],
    ...
    switch (stage) {
      ...
      case "provision":
        newRow = {
          ...
          encryptionMethod: "",
          provisionBasis: "",
          provisionSystemOnline: "",
          provisionSystemEncrypted: "",
          recipientOnline: "",
          recipientEncrypted: "",
        };
        break;
      case "disposal":
        newRow = {
          ...
          disposalManager: "",
          disposalProcedure: "",
          separateStorageSpace: "",
          disposalOnline: "",
          hasSeparateStorage: "",
          separateStorageOnline: "",
          separateStorageEncrypted: "",
        };
        break;
    }
  ...
  const handleEdit = (
    stage: "collection" | "storage" | "usage" | "provision" | "disposal",
    id: string,
    field: string,
    value: string,
  ) => {
    setFlowDataByTask((prev) => {
      ...
      return {
        ...
        [selectedTask]: {
          ...currentTask,
          [stage]: currentStageData.map((item: any) => (item.id === id ? { ...item, [field]: value } : item)),
        },
      };
    });
  };
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      {/* 고정된 헤더 영역 */}
      <div className="flex-shrink-0 bg-background border-b pb-4 space-y-4">
        ...
        <Tabs value={selectedTask} onValueChange={setSelectedTask} className="px-6">
          <TabsList>
            {taskNames.map((t) => (
      ...
      <div className="flex-1 overflow-auto px-6 py-6">
        <Tabs value={selectedTask} onValueChange={setSelectedTask}>
          {taskNames.map((task) => (
            <TabsContent key={task} value={task} className="space-y-6 m-0">
              {/* 수집 단계 */}
              <Card>
                    ...
                    <TableBody>
                      {flowDataByTask[task]?.collection.map((row) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell>
                            <Input
                              value={row.detailTask}
                              onChange={(e) => handleEdit("collection", row.id, "detailTask", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.collectionTarget}
                              onChange={(e) => handleEdit("collection", row.id, "collectionTarget", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.collectionPath}
                              onChange={(e) => handleEdit("collection", row.id, "collectionPath", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.collectionSystem}
                              onChange={(e) => handleEdit("collection", row.id, "collectionSystem", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.collectionItem}
                              onChange={(e) => handleEdit("collection", row.id, "collectionItem", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.collectionPeriod}
                              onChange={(e) => handleEdit("collection", row.id, "collectionPeriod", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.collectionManager}
                              onChange={(e) => handleEdit("collection", row.id, "collectionManager", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.collectionBasis}
                              onChange={(e) => handleEdit("collection", row.id, "collectionBasis", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Select
                              value={row.isOnline}
                              onValueChange={(value) => handleEdit("collection", row.id, "isOnline", value)}
                            >
                              <SelectTrigger>
                          ...
                          <TableCell>
                            <Select
                              value={row.isEncrypted}
                              onValueChange={(value) => handleEdit("collection", row.id, "isEncrypted", value)}
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
                              </SelectTrigger>
                      ...
                      {flowDataByTask[task]?.storage.map((row) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell>
                            <Input
                              value={row.detailTask}
                              onChange={(e) => handleEdit("storage", row.id, "detailTask", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.inputSystem}
                              onChange={(e) => handleEdit("storage", row.id, "inputSystem", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.storageSpace}
                              onChange={(e) => handleEdit("storage", row.id, "storageSpace", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.storageItem}
                              onChange={(e) => handleEdit("storage", row.id, "storageItem", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.encryptionItem}
                              onChange={(e) => handleEdit("storage", row.id, "encryptionItem", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Select
                              value={row.isOnline}
                              onValueChange={(value) => handleEdit("storage", row.id, "isOnline", value)}
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
                              </SelectTrigger>
                          ...
                          <TableCell>
                            <Select
                              value={row.isEncrypted}
                              onValueChange={(value) => handleEdit("storage", row.id, "isEncrypted", value)}
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
                              </SelectTrigger>
                      ...
                      {flowDataByTask[task]?.usage?.map((row) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell>
                            <Input
                              value={row.detailTask}
                              onChange={(e) => handleEdit("usage", row.id, "detailTask", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.storageSpace}
                              onChange={(e) => handleEdit("usage", row.id, "storageSpace", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.usageSystem}
                              onChange={(e) => handleEdit("usage", row.id, "usageSystem", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.usageItem}
                              onChange={(e) => handleEdit("usage", row.id, "usageItem", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.usagePurpose}
                              onChange={(e) => handleEdit("usage", row.id, "usagePurpose", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.usageMethod}
                              onChange={(e) => handleEdit("usage", row.id, "usageMethod", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.personalInfoHandler}
                              onChange={(e) => handleEdit("usage", row.id, "personalInfoHandler", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Select
                              value={row.isOnline}
                              onValueChange={(value) => handleEdit("usage", row.id, "isOnline", value)}
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
                              </SelectTrigger>
                          ...
                          <TableCell>
                            <Select
                              value={row.isEncrypted}
                              onValueChange={(value) => handleEdit("usage", row.id, "isEncrypted", value)}
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
                              </SelectTrigger>
                      ...
                      {flowDataByTask[task]?.provision.map((row) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell>
                            <Input
                              value={row.detailTask}
                              onChange={(e) => handleEdit("provision", row.id, "detailTask", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.storageSpace}
                              onChange={(e) => handleEdit("provision", row.id, "storageSpace", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.provisionSystem}
                              onChange={(e) => handleEdit("provision", row.id, "provisionSystem", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.provider}
                              onChange={(e) => handleEdit("provision", row.id, "provider", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.recipient}
                              onChange={(e) => handleEdit("provision", row.id, "recipient", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.provisionItem}
                              onChange={(e) => handleEdit("provision", row.id, "provisionItem", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.provisionPurpose}
                              onChange={(e) => handleEdit("provision", row.id, "provisionPurpose", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.provisionMethod}
                              onChange={(e) => handleEdit("provision", row.id, "provisionMethod", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.provisionPeriod}
                              onChange={(e) => handleEdit("provision", row.id, "provisionPeriod", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.encryptionMethod}
                              onChange={(e) => handleEdit("provision", row.id, "encryptionMethod", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.provisionBasis}
                              onChange={(e) => handleEdit("provision", row.id, "provisionBasis", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Select
                              value={row.provisionSystemOnline}
                              onValueChange={(value) => handleEdit("provision", row.id, "provisionSystemOnline", value)}
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
                              </SelectTrigger>
                          ...
                          <TableCell>
                            <Select
                              value={row.provisionSystemEncrypted}
                              onValueChange={(value) =>
                                handleEdit("provision", row.id, "provisionSystemEncrypted", value)
                              }
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
                          ...
                          <TableCell>
                            <Select
                              value={row.recipientOnline}
                              onValueChange={(value) => handleEdit("provision", row.id, "recipientOnline", value)}
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
                              </SelectTrigger>
                          ...
                          <TableCell>
                            <Select
                              value={row.recipientEncrypted}
                              onValueChange={(value) => handleEdit("provision", row.id, "recipientEncrypted", value)}
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
                              </SelectTrigger>
                      ...
                      {flowDataByTask[task]?.disposal.map((row) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell>
                            <Input
                              value={row.detailTask}
                              onChange={(e) => handleEdit("disposal", row.id, "detailTask", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.storageSpace}
                              onChange={(e) => handleEdit("disposal", row.id, "storageSpace", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.disposalSystem}
                              onChange={(e) => handleEdit("disposal", row.id, "disposalSystem", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.disposalPeriod}
                              onChange={(e) => handleEdit("disposal", row.id, "disposalPeriod", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.disposalItem}
                              onChange={(e) => handleEdit("disposal", row.id, "disposalItem", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.retentionPeriod}
                              onChange={(e) => handleEdit("disposal", row.id, "retentionPeriod", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.disposalManager}
                              onChange={(e) => handleEdit("disposal", row.id, "disposalManager", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.disposalProcedure}
                              onChange={(e) => handleEdit("disposal", row.id, "disposalProcedure", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              value={row.separateStorageSpace}
                              onChange={(e) => handleEdit("disposal", row.id, "separateStorageSpace", e.target.value)}
                            />
                          </TableCell>
                          <TableCell>
                            <Select
                              value={row.disposalOnline}
                              onValueChange={(value) => handleEdit("disposal", row.id, "disposalOnline", value)}
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
                              </SelectTrigger>
                          ...
                          <TableCell>
                            <Select
                              value={row.hasSeparateStorage}
                              onValueChange={(value) => handleEdit("disposal", row.id, "hasSeparateStorage", value)}
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
                              </SelectTrigger>
                          ...
                          <TableCell>
                            <Select
                              value={row.separateStorageOnline}
                              onValueChange={(value) => handleEdit("disposal", row.id, "separateStorageOnline", value)}
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
                              </SelectTrigger>
                          ...
                          <TableCell>
                            <Select
                              value={row.separateStorageEncrypted}
                              onValueChange={(value) =>
                                handleEdit("disposal", row.id, "separateStorageEncrypted", value)
                              }
                            >
                              <SelectTrigger>
                                <SelectValue placeholder="선택" />
