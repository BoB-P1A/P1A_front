...
export default function ProtectionReport() {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                setTaskTableData(Array.isArray(tasks) ? tasks : []);
    ...
    const handleDownload = async () => {
        try {
            ...
            const resultsByTask: {
                [key: string]: { [field: string]: { 이행: number, 부분이행: number, 미이행: number, 해당없음: number } }
            } = {};
            if (Array.isArray(lifecycleData)) {
                lifecycleData.forEach((item: any) => {
                                    ...
                                    <div key={phase} className="space-y-2">
                                        ...
                                        <div className="overflow-x-auto">
                                            <UITable>
                                                <UITableHeader>
                                                    <UITableRow>
                                                        {headers.map((h) => <UITableHead key={h}
                                                                                         className="text-center min-w-[80px]">{renderHeader(h)}</UITableHead>)}
                                                    </UITableRow>
                                                </UITableHeader>
                                                <UITableBody>
                        ...
                        <div className="space-y-4">
                            ...
                            {(() => {
                                ...
                                if (Object.keys(flowChartImages).length === 0) {
                                    return <p className="text-sm text-muted-foreground">각 처리업무별 흐름도는 "개인정보 흐름도" 페이지에서
                                        "이미지 저장" 버튼을 클릭하여 저장하세요.</p>;
                                }
                                ...
                                <div key={task} className="space-y-1">
                                    ...
                                    <ul className="list-disc pl-6">
                                        {Object.keys(resultsByTask[task]).map((field) => {
                                            ...
                                            return (
                                                <li key={field}>{field}: 이행 {c.이행}건, 부분이행 {c.부분이행}건, 미이행 {c.미이행}건,
                                                    해당없음 {c.해당없음}건 (이행률: {rate}%)</li>
                                            );
                                        })}
                                    </ul>
                                </div>
